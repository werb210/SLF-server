// SLF_FULL_MODEL_v1 - read API for the portal's SLF silo.
...
export function slfRouter() {
  ...
  r.get("/deals", async (req, res, next) => {
    try {
      const { stage, family, sub, q } = req.query as Record<
        string,
        string | undefined
      >;
      ...
      if (stage) {
        p.push(stage);
        where.push(`r.stage = $${p.length}`);
      }
      if (family) {
        p.push(family);
        where.push(`r.product_family = $${p.length}`);
      }
      if (sub) {
        p.push(Number(sub));
        where.push(`r.sub_id = $${p.length}`);
      }
      if (q) {
        p.push(`%${q}%`);
        where.push(
          `(s.company_name ILIKE $${p.length} OR r.company_name ILIKE $${p.length})`,
        );
      }
      p.push(limit);
      const lim = p.length;
  ...
  r.get("/deals/:id", async (req, res, next) => {
    try {
      ...
      const [reqRow, sub, contracts, offers, files] = await Promise.all([
        ...
        pool.query(
          `SELECT * FROM slf_contracts WHERE request_id = $1 ORDER BY id`,
          [id],
        ),
        pool.query(
          `SELECT * FROM slf_offers WHERE request_id = $1 ORDER BY slf_created_at DESC NULLS LAST, id DESC`,
          [id],
        ),
        pool.query(
      ...
      const subRow =
        ...
        (requestRow.company_name
          ...
          : null);
      const users = sub.rows[0]
        ? (
            await pool.query(
              `SELECT * FROM slf_users WHERE sub_id = $1 ORDER BY id`,
              [sub.rows[0].id],
            )
          ).rows
        : [];
      const notes = [
        ...(requestRow.notes
          ? [{ source: "request", author: null, text: requestRow.notes }]
          : []),
        ...(sub.rows[0]?.notes
          ? [
              {
                source: "sub",
                author: sub.rows[0].company_name,
                text: sub.rows[0].notes,
              },
            ]
          : []),
        ...contracts.rows
        ...
        ...offers.rows
          ...
          .map((o) => ({
            source: "offer",
            author: o.lender_name,
            text: o.notes,
          })),
        ...offers.rows
          .filter((o) => o.reject_reason)
          .map((o) => ({
            source: "offer_reject",
            author: o.lender_name,
            text: o.reject_reason,
          })),
      ];
      res.json({
        success: true,
  ...
  r.get("/lenders", async (_req, res, next) => {
    try {
      const { rows } = await pool.query(
        `SELECT lender_id, lender_name, max(lender_logo) AS lender_logo, count(*) AS offer_count, count(*) FILTER (WHERE status = 'accepted') AS accepted_count, count(*) FILTER (WHERE reject_reason <> '') AS rejected_count, coalesce(sum(amount) FILTER (WHERE status = 'accepted'),0) AS accepted_total FROM slf_offers WHERE lender_id IS NOT NULL GROUP BY lender_id, lender_name ORDER BY lender_name`,
      );
      res.json({ success: true, data: rows });
    } catch (e) {
      next(e);
  ...
  // SLF_BROKER_OF_RECORD_v1 - broker-of-record view. We are the broker and the
  // SLF API is read-only, so these are live passthroughs (small payloads, no
  // mirror table needed). /broker/summary returns one row per transaction with
  // {product,status,fininst,amount,date,percent,total} across all families.
  r.get("/broker", async (_req, res, next) => {
    try {
      const { data } = await slfClient.get("/api/broker/");
      res.json({ success: true, data });
    } catch (e) {
      next(e);
    }
  });
  r.get("/broker/fi", async (_req, res, next) => {
    try {
      const { data } = await slfClient.get("/api/broker/fi/");
      res.json({ success: true, data });
    } catch (e) {
      next(e);
    }
  });
  r.get("/broker/summary", async (_req, res, next) => {
    try {
      const { data } = await slfClient.get("/api/broker/summary/");
      const rows = Array.isArray(data) ? data : [];
      const totals = rows.reduce(
        (
          acc: { count: number; amount: number; total: number },
  ...
  r.post("/sync", async (_req, res, next) => {
    try {
