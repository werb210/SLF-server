...
describe("ingestRequest", () => {
  ...
  it("never overwrites a request id owned by another family", async () => {
    h.query.mockImplementation(async (sql: string) =>
      sql.startsWith("INSERT INTO slf_requests") ? { rowCount: 0 } : { rowCount: 1 },
    );
    await ingestRequest("invoice", {
      id: 1,
      contract: { id: 1, contractNumber: "DAIMOND 01" },
    });
    const req = sqlCalls().find((s) => s.startsWith("INSERT INTO slf_requests"))!;
    expect(req).toContain("WHERE slf_requests.product_family = EXCLUDED.product_family");
    expect(sqlCalls()).toContain("ROLLBACK");
    expect(sqlCalls()).not.toContain("COMMIT");
    expect(sqlCalls().some((s) => s.includes("slf_contracts"))).toBe(false);
  ...
  it("stores an invoice's single contract object", async () => {
    ...
    await ingestRequest("invoice", {
      ...
      contract: { id: 1, contractNumber: "DAIMOND 01", amount: 138330, generalContractor: "AKIVA DAIMOND" },
    });
    const call = h.query.mock.calls.find(([s]) => String(s).includes("INTO slf_contracts"));
