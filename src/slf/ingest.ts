...
async function upsertSub(
  c: PoolClient,
  sub: Json | null | undefined,
): Promise<number | null> {
  if (!sub || typeof sub.id !== "number") return null;
  const subId: number = sub.id;
  await upsertUser(c, sub.admin, subId);
  await upsertUser(c, sub.applicant, subId);
  if (Array.isArray(sub.users))
    for (const u of sub.users) await upsertUser(c, u, subId);
  await c.query(
    ...
    [
      subId,
      sub.companyName ?? null,
      sub.gst ?? null,
      toNum(sub.employeeCount),
      sub.address ?? null,
      sub.city ?? null,
      sub.province ?? null,
      sub.postalCode ?? null,
      sub.country ?? null,
      sub.website ?? null,
      Array.isArray(sub.subTrade) ? sub.subTrade.map(String) : null,
      toBool(sub.union),
      toBool(sub.subContracting),
      toBool(sub.edSub),
      toBool(sub.isApproved),
      sub.partnership ?? null,
      sub.notes ?? null,
      toBool(sub.businessBankruptcy),
      toBool(sub.personalBankruptcy),
      sub.companyStart ?? null,
      sub.admin?.id ?? null,
      sub.applicant?.id ?? null,
      sub.applicantJobTitle ?? null,
      toNum(sub.applicantOwnershipPercent),
      sub.applicantInvited ?? null,
      toBool(sub.applicantFormCompleted),
      JSON.stringify(sub.requestedDocuments ?? []),
      JSON.stringify(sub.requiredDocuments ?? []),
      sub.updatedAt ?? null,
      JSON.stringify(sub),
    ],
  );
  await upsertFiles(c, sub.files, "sub", subId, null);
  return subId;
}
...
export async function ingestRequest(family: string, req: Json): Promise<void> {
  ...
  try {
    await c.query("BEGIN");
    const subId = await upsertSub(c, req.sub);
    ...
    await c.query(
      ...
      [
        ...
        extractCountry(req),
        toBool(req.isActive),
        toBool(req.isComplete),
        toBool(req.hidden),
        toBool(req.offered),
        toNum(req.ongoingLoc?.count),
        toNum(req.ongoingLoc?.total),
        req.equipmentFinanceRequest
          ? JSON.stringify(req.equipmentFinanceRequest)
          : null,
        stage,
        JSON.stringify(req),
        extractCompanyName(req),
        extractExternalStatus(req),
        lender.name,
        lender.logo,
        terms.invoiceTotal,
        terms.invoiceCount,
    ...
    for (const o of Array.isArray(req.offer) ? req.offer : []) {
      ...
      await c.query(
        ...
        [
          ...
          o.rejectReason ?? null,
          o.notes ?? null,
          toNum(o.originalInterestRate),
          o.interestRateType ?? null,
          o.fininst?.id ?? null,
          o.fininst?.name ?? null,
          o.fininst?.logo ?? null,
          toBool(o.ended),
      ...
      await upsertFiles(c, o.terms, "offer_terms", o.id, requestId);
      await upsertFiles(
        c,
        o.signedTermsFiles,
        "offer_signed_terms",
      ...
      await upsertFiles(c, o.files, "offer_file", o.id, requestId);
      await upsertFiles(c, o.signedFiles, "offer_signed_file", o.id, requestId);
      await upsertFiles(
        c,
        o.additionalFiles,
        "offer_additional",
    ...
    for (const field of REQUEST_FILE_FIELDS) {
      await upsertFiles(
        c,
        (req as Json)[field],
        `request_${field}`,
